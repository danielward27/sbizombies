...
```{r}
ggplot(df, aes(true_parameters, inferred_parameter_value)) +
  geom_point() +
  facet_grid(algorithm~parameter, scales = "fixed") +
  geom_abline(slop=1, colour = "brown") +
  labs(x = "True parameter value", y = "Inferred parameter value")
```
```{r}
ggsave("plots/true_vs_predicted.png")
```
...
```{r}
ggplot(df, aes(y = rmse, x = algorithm)) +
  geom_boxplot(fill = "lightgrey") +
  facet_wrap(~parameter) +
  stat_summary(fun.y=mean, geom="point", shape=4, size=3, color="black", fill="red") +
  labs(x = "Algorithm", y = "Root mean square error")
```
```{r}
ggsave("plots/rmse_boxplot.png")
```
